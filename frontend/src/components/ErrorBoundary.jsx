                    ...
                    <div className="text-center max-w-md w-full p-8 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl">
                        ...
                        {this.props.showDetails && this.state.error && (
                            <details className="text-left mb-6 bg-black/40 p-4 rounded-lg max-h-48 overflow-auto border border-white/5">
                                <summary className="cursor-pointer text-slate-300 font-medium mb-2 hover:text-white transition-colors">Error Details</summary>
                                <pre className="text-xs text-red-300 whitespace-pre-wrap break-all font-mono">{this.state.error.toString()}</pre>
                                {this.state.errorInfo && (
                                    <pre className="text-xs text-slate-500 mt-2 whitespace-pre-wrap break-all font-mono">{this.state.errorInfo.componentStack}</pre>
                                )}
                            </details>
                        )}
